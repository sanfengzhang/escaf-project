...
import com.escaf.esservice.search.geo.GeoPoint;
import com.escframework.es.anotation.CompletionField;
import com.escframework.es.anotation.DateFormat;
...
import com.escframework.es.anotation.GeoPointField;
import com.escframework.es.anotation.InnerField;
import com.escframework.es.anotation.Mapping;
import com.escframework.es.anotation.MultiField;
import com.escframework.es.anotation.Parent;
...
public class MappingBuilder
{
	...
	public static XContentBuilder buildMapping(Class<?> clazz, String indexType) throws IOException
	{
		...
		for (java.lang.reflect.Field field : fields)
		{
			...
			boolean isId = field.getAnnotation(Field.class).isId();

			if (null == idFieldName && isId)
			{
				idFieldName = field.getName();
			}
			boolean hasParentType = field.isAnnotationPresent(Parent.class);
			if (hasParentType && parentType == null)
			{
				parentType = field.getAnnotation(Parent.class).type();
			}
	...
	private static void applyCompletionFieldMapping(XContentBuilder xContentBuilder, java.lang.reflect.Field field, CompletionField annotation)
			throws IOException
	{
		xContentBuilder.startObject(field.getName());
	 ...
	 * @throws IOException
	 */
	private static void addSingleFieldMapping(XContentBuilder xContentBuilder, java.lang.reflect.Field field, Field fieldAnnotation,
		...
		if (FieldType.Auto != fieldAnnotation.type())
		{
			...
			if (FieldType.Date == fieldAnnotation.type() && DateFormat.none != fieldAnnotation.format())
			{
				xContentBuilder.field(FIELD_FORMAT,
						DateFormat.custom == fieldAnnotation.format() ? fieldAnnotation.pattern() : fieldAnnotation.format());
			}
		}
